...
export type { Provider } from '@monobase/sdk/types'
...
/**
 * Guard that requires user to have a provider profile
 * Reads auth state from router context - no querying!
 * Redirects to onboarding if provider profile doesn't exist
 */
export async function requireProviderProfile({ context }: { context: RouterContext }) {
  if (!context.auth.provider) {
    throw redirect({
      to: '/onboarding' as any,
    })
  }
  return { provider: context.auth.provider }
}

/**
 * Guard that requires user to NOT have a provider profile
 * Reads auth state from router context - no querying!
 * Used for onboarding flow - redirects to dashboard if provider profile already exists
 */
export async function requireNoProviderProfile({ context }: { context: RouterContext }) {
  if (context.auth.provider) {
    throw redirect({
      to: '/dashboard',
    })
 ...
 *
 * @example
 * beforeLoad: composeGuards(requireAuth, requirePerson, requireProviderRole)
 */
