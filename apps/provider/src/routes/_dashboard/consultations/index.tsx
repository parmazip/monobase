...
type ConsultationStatus = 'draft' | 'finalized' | 'amended'
...
function ConsultationsPage() {
  ...
  const { data: consultationsData, isLoading: consultationsLoading, error } = useConsultations({
    limit: 100,
  })
  ...
  const filteredNotes = useMemo(() => {
    let notes =
      selectedStatus === 'draft'        ? draftNotes
    ...
    if (searchQuery) {
      notes = notes.filter(
        (note) =>
          ...
          patientMap.get(note.patient)?.toLowerCase().includes(searchQuery.toLowerCase())
      )
    }

    return notes
  }, [allNotes, draftNotes, finalizedNotes, selectedStatus, searchQuery, patientMap])
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] p-6">
        ...
        <p className="mt-4 text-muted-foreground">Loading consultation notes...</p>
      </div>
    )
  }
  ...
  if (error) {
        ...
        <Card className="border-red-200 bg-red-50">
          <CardContent className="p-6">
            <div className="flex items-center gap-2 text-red-800">
              ...
              <p className="font-semibold">Failed to load consultation notes</p>
            </div>
    ...
    <div className="flex flex-col gap-6 p-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-3xl font-headline font-bold">Consultation Notes</h1>
          <p className="text-muted-foreground font-body">
            Manage patient consultation notes and medical documentation
          </p>
        </div>
        <Button>
        ...
        <CardContent className="p-4">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
            <div className="relative flex-1 max-w-md">
              ...
              <Input
                placeholder="Search by patient, complaint, or assessment..."
                value={searchQuery}
        ...
        {filteredNotes.map((note) => (
          <ConsultationCard
            ...
            consultation={{
              ...
              patientName: patientMap.get(note.patient) || 'Unknown Patient',
              providerId: note.provider,
              date: note.createdAt,
              status: note.status as ConsultationStatus,
      ...
      {filteredNotes.length === 0 && (
        <Card>
          <CardContent className="p-12 text-center">
            ...
            <h3 className="text-lg font-semibold mb-2">No consultation notes found</h3>
