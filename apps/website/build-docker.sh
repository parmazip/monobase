...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    --help|-h)
      ...
      echo ""
      echo "Build and optionally push Docker image for website app"
      echo ""
      echo "Options:"
      echo "  --skip-build-assets     Skip building assets (assumes .next exists)"
      ...
      echo "  $0 --skip-build-assets --push         # Build and push image (assumes .next exists)"
...
echo -e "${BLUE}╔════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║  Docker Build Script - Website App    ║${NC}"
echo -e "${BLUE}╚════════════════════════════════════════╝${NC}"
echo ""
...
# Check if .next directory exists (Next.js output)
if [ ! -d ".next" ] && [ "$DRY_RUN" = false ]; then
