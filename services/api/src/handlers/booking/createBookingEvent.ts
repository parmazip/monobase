...
export async function createBookingEvent(ctx: Context) {
  ...
  // Generate initial slots for this event
  try {
    await regenerateOwnerSlots(db, user.id);
    logger?.info({ eventId: event.id, ownerId: user.id }, 'Initial slots generated successfully');
  } catch (error) {
    // Log but don't fail the request - slots can be generated by the background job
    logger?.error({
      eventId: event.id,
      ownerId: user.id,
      error: error instanceof Error ? error.message : 'Unknown error'
    }, 'Failed to generate initial slots - will be handled by background job');
  }
